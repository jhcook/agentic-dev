...
2. **Add AI** with `agent impact <STORY-ID>` for AI-powered risk assessment.
3. **Update story** with `agent impact <STORY-ID> --update-story` to inject analysis.
...
| Flag | Description |
|------|-------------|
| `[AI Default]` | Enable AI-powered risk assessment (default: static only) |
...
- The `--offline` flag requires valid AI provider credentials
