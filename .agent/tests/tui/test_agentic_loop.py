...
class TestLocalToolClient:
    ...
    async def test_lists_all_expected_tools(self, repo):
        """@QA: Should register 9 tools (5 read-only + 4 interactive)."""
...
class TestRunAgenticLoop:
    ...
    async def test_yields_tool_call_events(self, mock_ai, mock_executor_cls, mock_client_cls):
        ...
        assert result == "Done."
        on_tool_call.assert_called_once_with("read_file", {"path": "foo.py"})
        on_tool_result.assert_called_once_with("read_file", "file contents")
