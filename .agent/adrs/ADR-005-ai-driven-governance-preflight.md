...
1. **False Positives**: LLMs may flag innocent code as violations (e.g., flagging a variable named `password_hash` in a test file as a leak).
   - **Mitigations** (layered, in order of cost):
     1. **Suppression rules** in the system prompt (8 rules covering common false-positive categories including diff context limitations). Always active.
     2. **Expanded diff context** (`-U10` instead of default `-U3`), tripling the window around each change. Always active.
     3. **Full-file context augmentation** (`--thorough`): AST-extracted function/class signatures injected into the prompt, giving the AI structural awareness beyond the diff.
     4. **Post-processing validator** (`--thorough`): Cross-references findings against actual source code; filters claims about "missing" code when the code exists in the file.
2. **Cost/Latency**: Running 5-10 concurrent LLM calls per commit can be slow and incur API costs. The `--thorough` flag gates token-intensive features (mitigations 3-4 above) so they are opt-in.
