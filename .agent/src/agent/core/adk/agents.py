...
def create_role_agent(role: Dict, tools: List, model, other_domains: str = "") -> LlmAgent:
    ...
    system_instruction = (
        ...
        f"SCOPE RULES (non-negotiable):\n"
        f"1. Only raise a finding if you can cite its exact source file and line. "
        f"   Do NOT raise findings based on assumptions about what might be missing.\n"
        f"2. If the story's Impact Analysis section already documents a change as "
        f"   'out-of-scope but co-committed' or credits it to another story, "
        f"   do NOT re-raise it as a scope violation. It has been acknowledged.\n"
        f"3. Do NOT contradict a finding that was resolved in a previous run. "
        f"   If the <previous_verdicts> block shows a finding was addressed, "
        f"   accept that resolution and move on.\n"
        f"4. If you are unsure whether a finding is in scope for this story, "
        f"   default to PASS with a note rather than BLOCK.\n\n"
