# STORY-ID: INFRA-111: Extract Prompt and Command Logic
...
Decompose the `tui/app.py` monolith by extracting prompt-building logic into a dedicated `agent/tui/prompts.py` module. Command and parsing logic has already been extracted to `commands.py` in a separate effort. This isolation separates prompt templating and message history management from the Textual UI framework, improving maintainability.
...
### @Architect
- **ADR Compliance**: Follows ADR-041 (Module Decomposition Standards) by extracting domain logic from UI components.
- **Boundaries**: Successfully removes prompt building logic from `app.py`.

### @Qa
- **Test Strategy**: The move is purely structural. No behavioral changes.
- **Coverage**: Acceptance criteria are satisfied by the structural changes.

## Implementation Steps

### Step 1: Create the new prompts module

#### [NEW] src/agent/tui/prompts.py
...
### Step 2: Remove prompts and imports from app.py
...
### Automated Tests
- Run `pytest tests/tui` or `uv run pytest .agent/tests`

### Manual Verification
- Run `agent console` and test that the TUI comes up properly with the system prompt and welcome message intact.

## Definition of Done
### Testing
- [x] Application behaves normally
