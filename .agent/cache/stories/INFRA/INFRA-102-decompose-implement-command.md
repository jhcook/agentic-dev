...
- [ ] **AC-10**: Both the full-context and chunked system prompts include a `CRITICAL — DOCSTRINGS` rule stating that every module, class, and function/method (including inner decorator closures) MUST have a PEP-257 docstring, and that missing docstrings will BLOCK the build. This prevents the AI from generating undocumented code that fails the `docs_check` gate on every run.
