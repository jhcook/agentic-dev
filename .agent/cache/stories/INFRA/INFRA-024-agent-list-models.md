...
## User Story
As a developer, I want to use `env -u VIRTUAL_ENV uv run agent list-models <provider>` so that I can immediately see which models are available for me to use in my configuration.

## Acceptance Criteria
- [ ] **Scenario 1**: `env -u VIRTUAL_ENV uv run agent list-models gemini` lists all available Gemini models (utilizing the API method `client.models.list()`).
- [ ] **Scenario 2**: `env -u VIRTUAL_ENV uv run agent list-models openai` lists available OpenAI models (utilizing `client.models.list()`).
- [ ] **Scenario 3**: `env -u VIRTUAL_ENV uv run agent list-models gh` lists available GitHub Models (via `gh` CLI).
- [ ] **Scenario 4**: `env -u VIRTUAL_ENV uv run agent list-models` (without args) lists models for the currently active/default provider.
