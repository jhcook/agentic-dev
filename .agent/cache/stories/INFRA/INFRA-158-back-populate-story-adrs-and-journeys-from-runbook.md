...
- **INFRA-094 and others**: Multiple tests and files modified/added in this branch (`.agent/tests/journeys/test_jrn_*.py`, `.agent/tests/commands/test_runbook_split_request.py`, `.agent/tests/governance/test_syntax_validation.py`, etc.) are explicitly noted as out-of-scope but co-committed to finalize the branch preflight.
- **`.agent/src/agent/commands/check.py`**: Three improvements co-committed: (1) `InteractiveFixer` integration — when story schema validation fails and `--interactive` is set, the preflight command invokes `InteractiveFixer.analyze_failure("story_schema", ...)` to surface fix options (with voice-mode formatting support); (2) `AGENT_SKIP_KEYRING=1` is propagated into pytest subprocess environments to prevent macOS keychain dialogs from blocking CI; (3) `subprocess.TimeoutExpired` handling added to prevent indefinite hangs.
