...
### Features
- **(INFRA-024)** Added `agent list-models [provider]` command to query available AI models from any configured provider (Gemini, OpenAI, Anthropic, GitHub CLI). Supports multiple output formats (`--format json/csv/yaml/markdown`) and file output (`--output`).
...
- **(INFRA-000)** Enhanced `agent preflight` with Smart Test Selection. Now identifies relevant tests based on dependency analysis of changed files. Supports monorepo structures (Python/backend, React Native/mobile, Next.js/web) and adds `--skip-tests` / `--ignore-tests` flags.
